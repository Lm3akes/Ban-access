...
BOT_TOKEN = "8493230010:AAEpdGI-qkS7we_gejKIB9JUcXN4nrkfwRU"
...
def send_welcome(message):
    bot.reply_to(message, "مرحبا! استخدم الأمر /ban متبوعا بـ access token")

@bot.message_handler(commands=['ban'])
def ban_command(message):
    try:
        access_token = message.text.split(' ', 1)[1].strip()
        if not access_token:
            bot.reply_to(message, "الرجاء إدخال access token")
            return
    except IndexError:
        bot.reply_to(message, "الاستخدام: /ban {access_token}")
        return
    ...
    if success:
        bot.edit_message_text(f"{result}", status_msg.chat.id, status_msg.message_id)
    else:
        bot.edit_message_text(f" {result}", status_msg.chat.id, status_msg.message_id)
